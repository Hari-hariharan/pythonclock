...
def clock():
        ...
        if int(hour) > 11 and int(hour) < 24:                                                        #print while the time is PM
                time = str(int(hour) - 12) + ':' + minutes + ':' + seconds + ' ' + time3
        else:
                time = time2 + ' ' + time3                                                          #print while the time is AM
        time_label.config(text = time)
        date_label.config(text= date)
        time_label.after(1000, clock)
def alarm():
        main_time = datetime.datetime.now().strftime("%H:%M %p")                       #make a alarm function that controls the Time and Date Labels
        ...
        main_hour1, main_minutes = main_time1.split(':')                               #split the given input
        if main_time2 == 'PM':
                main_hour = str(int(main_hour1) - 12)                                  #when the time is PM subtract by 12 
        else:
                main_hour = main_hour1                                                  #when the time is AM
        if int(alarm_hour) == int(main_hour) and int(alarm_minutes) == int(main_minutes) and main_time2 == alarm_time2:       #To  check when the given time is equal to actual time
                for i in range(3):
...
def remainder():
        main_time = datetime.datetime.now().strftime("%H:%M %p")                                     #make a remainder function that controls the Time and Date Labels
        ...
        if int(remainder_hour) == int(main_hour) and int(remainder_minutes) == int(main_minutes):              #To  check when the given time is equal to actual time
                ...
                set_remainder_button.config(state='enabled')                                                        #to enable the button
...
remainder_instructions_label.pack(anchor='s')
get_remainder_time_entry2 = Entry(remainder_tab, font = 'calibri 15 bold')       #Here to get input box
get_remainder_time_entry2.pack(anchor='center')
remainder_instructions_label = Label(remainder_tab, font = 'calibri 10 bold', text = "Enter Remainder Time. Eg -> 01:30 PM, 01 -> Hour, 30 -> Minutes")    #Here we added a Labels to provide the information in the tab
remainder_instructions_label.pack(anchor='s')
